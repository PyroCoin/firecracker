...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--sender', default="ignore", type=str, help='sender')
    parser.add_argument('-r', '--recipient', default="ignore", type=str, help='recipient')
    ...
    if args.recipient != "ignore":
        recipient = SigningKey.from_string(bytes.fromhex(args.recipient))
    ...
    transaction['signature'] = sender.sign(f"{s_pub_key} -{args.amount}-> {r_pub_key}".encode()).hex()
